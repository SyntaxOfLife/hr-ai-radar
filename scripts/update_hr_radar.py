...
SOURCE_PRIORS = {
    ...
    "hrzone": 0.5, "clo": 0.5, "worklife": 0.5, "littler": 0.6, "ela": 0.6, "aihot": 0.2
}
...
def is_hr_relevant(title, source_id=""):
    if source_id in ("hr_dive", "aihr", "hrzone", "hrexecutive", "clo", "worklife", "manual", "littler", "ela"):
        return True
    t = title.lower()
            ...
            with open(manual_path, "r", encoding="utf-8") as f:
                for item in json.load(f).get("items", []):
                    manual_items.append({
                        "title": item["title"],
                        "url": item["url"],
                        "published_at": item.get("date", (datetime.now(timezone.utc) - timedelta(days=5)).strftime("%Y-%m-%d")) + "T00:00:00+00:00",
                        "source_id": "manual",
