...
[![Live](https://img.shields.io/badge/Live-radar.learnprompt.pro-green?style=flat-square)](https://radar.learnprompt.pro)
...
**在线站** → [radar.learnprompt.pro](https://radar.learnprompt.pro)（数据源/备用：[learnprompt.github.io/ai-news-radar](https://learnprompt.github.io/ai-news-radar/)）
...
**② 直接看网页** → 打开 [radar.learnprompt.pro](https://radar.learnprompt.pro)。v0.8 起每条精选自带 persona 点评，每日 TOP3 还有实用派、毒舌评论员、论文警察三种口味并排锐评，同一条新闻三个角度一眼看完。
...
- 每日精选20条按默认口味打分点评，首页 TOP3 三味并排，一条新闻三个角度。
...
- 通过"伯乐精选"先看高价值故事线，TOP3 三口味锐评帮你快速定调
...
3. **（可选）配 `DEEPSEEK_API_KEY`**：Settings → Secrets and variables → Actions 加一个 secret，就能获得 persona 锐评和更好的中文标题翻译。不配也全流程能跑，自动降级成规则分加谷歌翻译。
...
想换信源：把订阅写进 `feeds/follow.opml`（参考 `feeds/follow.example.opml`），或让内置[伯乐Skill](skills/ai-news-radar/README.md)帮你判断和录入。想换口味：改 `personas/` 下的 markdown 文件。想要自己的域名：（可选）把仓库 import 进 Vercel，仓库里的 `vercel.json` 已配好，零构建直接上线。
...
- 如果设置 `DEEPSEEK_API_KEY`，会给每日精选打 persona 分并生成三口味 TOP3 点评；不设置时自动降级为规则分
...
- [AIHOT](https://aihot.virxact.com)：上游灵感与信源之一
